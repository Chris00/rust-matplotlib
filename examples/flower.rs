...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pseudacorus_Sepal_width = Series::new("Pseudacorus_Sepal_width",
        &[4.0,3.1,4.0,3.8,3.4,3.7,4.7,4.0,4.5,3.2,3.9,4.0,4.1,4.0,4.6,4.4,
            ...
            3.6,3.8]);
    let f: PolarsResult<DataFrame> =
        DataFrame::new(vec![pseudacorus_Sepal_Length,pseudacorus_Sepal_width]);
    match f {
        Ok(df) => {
            println!("df is {:?}", &df);
            ...
            let x_values: Vec<f64> =
                x_col.to_vec().iter().map(|&x| x.unwrap()).collect();
            let y_values: Vec<f64> =
                y_col.to_vec().iter().map(|&y| y.unwrap()).collect();
